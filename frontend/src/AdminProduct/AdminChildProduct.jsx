...
export default function AdminChildProduct() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ formData, isEdit, id }) => {
      const url = isEdit
        ? `${API_URL}/AdminChildProductUpdate/${id}`
        : `${API_URL}/AdminChildProductCreate`;
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const res = await fetch(`${API_URL}/AdminChildProductDelete/${id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-xl rounded-lg">
      ...
      <h1 className="text-2xl font-bold mb-4">
        {editingProduct ? "Edit Product" : "Add Child Product"}
      </h1>
