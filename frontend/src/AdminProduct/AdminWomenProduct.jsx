...
const API_URL = "http://localhost:5000/api/AdminWomenProduct";

const fetchProducts = async () => {
  const res = await fetch(`${API_URL}/AdminWomenProductGet`);
  const data = await res.json();
  if (!res.ok) throw new Error(data.message);
  return data;
};

export default function AdminWomenProduct() {
  ...
  const mutation = useMutation({
    mutationFn: async ({ formData, isEdit, id }) => {
      const url = isEdit
        ? `${API_URL}/AdminWomenProductUpdate/${id}`
        : `${API_URL}/AdminWomenProductCreate`;
  ...
  const deleteMutation = useMutation({
    mutationFn: async (id) => {
      const res = await fetch(`${API_URL}/AdminWomenProductDelete/${id}`, {
        method: "DELETE",
      });
      const json = await res.json();
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto bg-white shadow-xl rounded-lg">
      ...
      <h1 className="text-2xl font-bold mb-4">
        {editingProduct ? "Edit Product" : "Add Women Product"}
      </h1>
